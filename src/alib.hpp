...
inline bool operator==(const HSVA& lhs, const HSVA& rhs)
{
...
struct BasicAnim
{
	...
	friend struct AnimReflection;
	friend struct AnimDir;
};
...
struct AnimDir
{
	...
	std::vector<short> AllDirs() const
	{
	...
	std::pair<bool, BasicAnim*> Have(short dir);
...
private:
	...
	struct BadCmp {
		bool operator()(const BAD&, short);
		bool operator()(short, const BAD&);
...
struct AnimCollection
{
	...
	NAV* Have(std::string n) { if (mappings.count(n)==0) return nullptr; return mappings[n]; }
